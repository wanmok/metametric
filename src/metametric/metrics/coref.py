...
muc = mm.dataclass[EntitySet]({
    "entities": mm.set_matching[Entity, "~"](muc_link)
})
...
b_cubed_precision = mm.preprocess(
    ...
    mm.set_matching[Membership, "<->", "precision"](
        mm.dataclass[Membership]({
            "mention": ...,
            "entity": mm.normalize["precision"](mm.auto[Entity])
        })
    )
)
...
b_cubed_recall = mm.preprocess(
    ...
    mm.set_matching[Membership, "<->", "recall"](
        mm.dataclass[Membership]({
            "mention": ...,
            "entity": mm.normalize["recall"](mm.auto[Entity])
        })
    )
)
...
ceaf_phi4 = mm.dataclass[EntitySet]({
    "entities": mm.set_matching[Entity, "<->"](
        mm.dataclass[Entity]({
            "mentions": mm.set_matching[Mention, "<->", "f1"](...)
        })
    )
})
